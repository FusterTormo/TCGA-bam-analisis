...
#TODO somatic or germinal ???
strelka=~/soft/Strelka2.8.4/bin/configureStrelkaGermlineWorkflow.py
...
if (( $# == 2 )); then
        ...
        if (( $? == 0 )); then
			...
			#TODO change the output dir in case of somatic
                        $strelka --bam $1 --referenceFasta $reference --exome --indelCandidates $2/results/variants/candidateSmallIndels.vcf.gz --runDir $2+Strelka2
                        if (( $? == 0 )); then
                                echo -e "${GREEN}Strelka-2 configured successfully.${NC} Running the analysis\n\n"
                                #Idem de la ejecucion de Manta
                                $2+Strelka2/runWorkflow.py -m local -j 8 --quiet
                                if (( $? == 0 )); then
